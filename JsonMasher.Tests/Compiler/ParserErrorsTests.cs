    ...
    public class ParserErrorsTests
    {
        ...
        )
        {
            ...
            action.ShouldThrow<JsonMasherException>(e =>
                VerifyException(e, messageExpectation, positionInformationExpectation)
            );
        }

        private static IEnumerable<TestItem> GetTestData() =>
            Enumerable.Empty<TestItem>().Concat(ErrorTests());
...
"
            ...
            yield return new TestItem(
                ...
                new PositionInformation(1, 12, @"")
            );
            yield return new TestItem(
                "1 as",
                Messages.Parser.MatcherExpected,
                new PositionInformation(1, 5, @"")
            );
            yield return new TestItem(
                "1 as a",
                Messages.Parser.MatcherExpected,
...
"
            ...
            yield return new TestItem(
                "{ 2",
                "Expected ':', but reached end of input.",
                new PositionInformation(1, 4, @"")
            );
            yield return new TestItem(
                "{",
                "Unknown construct.",
                new PositionInformation(1, 2, @"")
            );
            yield return new TestItem(
                "{ a:2, }",
...
"
        ...
        {
            ex.Message.ShouldBe(messageExpectation);
            if (positionInformationExpectation != null)
            {
                ex.Line.ShouldBe(positionInformationExpectation.Line);
                ex.Column.ShouldBe(positionInformationExpectation.Column);
                ex.Highlights.CleanCR()
                    .ShouldBe(positionInformationExpectation.Highlights.CleanCR());
            }
            return true;
        }
    }
