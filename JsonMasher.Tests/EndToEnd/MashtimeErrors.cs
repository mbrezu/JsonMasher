...
namespace JsonMasher.Tests.EndToEnd
{
    public class MashtimeErrors
    {
        ...
        )
        {
            ...
            // Assert
            action.ShouldThrow<JsonMasherException>(ex =>
                ValidateException(ex, expectedMessage, expectedPositionInformation)
            );
        }
        ...
        )
        {
            ...
            {
                ex.Line.ShouldBe(expectedPositionInformation.Line);
                ex.Column.ShouldBe(expectedPositionInformation.Column);
                ex.Highlights.CleanCR().ShouldBe(expectedPositionInformation.Highlights.CleanCR());
            }
            return true;
        }

        private static IEnumerable<TestItem> GetTestData() =>
            Enumerable.Empty<TestItem>().Concat(SimplePrograms());
